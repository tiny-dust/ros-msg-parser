...
import { Field, Parse, ParseFileOptions, ParseResult } from './types';
...
export function genTsContent(params: Parse[]) {
  ...
  let output = `// Auto-generated by ros-to-ts\n\n`;
  params.forEach(item => {
    // 生成描述
    ...
    item.fields.forEach(field => {
      ...
      if (item.isEnum) {
        ...
        output += `  ${field.name}${item.required ? '' : '?'}: ${field.type}${field.isArray ? '[]' : ''};\n`;
      }
    });
    output += `}\n\n`;
  });
